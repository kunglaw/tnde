...
<?php
	...
	$sql  = " select kd_log_sk from log_sk ";

	//echo $sql;
	$num_array = mysqli_query($conn,$sql);
	$total_records = mysqli_num_rows($num_array);
	doPages($maxidx, 'index.php?link=surat_keluar/v_logsk', '', $total_records);
...
<table width="800" border="1" style="border-collapse:collapse; padding:7px;">
  ...
  <?php
	$queryCount = mysqli_query($conn," select count(kd_log_sk) from log_sk ");
	...
	else
	{
		...
		if($cari == "" || $tipe == "")
		{
			$sql  = " select * from log_sk 						";
			$sql .=	" order by 	kd_log_sk desc					";
			$sql .=	" limit $idx, $maxidx						";
			$queryTampil = mysqli_query($conn,$sql);
		}
		else
		{
			$sql  = " select * from log_sk where $tipe = '$cari' ";
			$sql .=	" order by kd_log_sk desc					      ";
			$sql .=	" limit $idx, $maxidx						      ";
			$queryTampil = mysqli_query($conn,$sql);
		}
